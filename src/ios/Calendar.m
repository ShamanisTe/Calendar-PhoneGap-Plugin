...
- (void) deleteEventFromCalendar:(CDVInvokedUrlCommand*)command
    ...
    NSDictionary* options = [command.arguments objectAtIndex:0];
    [self.commandDelegate runInBackground: ^{
        CDVPluginResult *pluginResult = [self deleteEventFromCalendarWithDictionnary:options calendar:calendar];
        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
    }];
}
- (CDVPluginResult*) deleteEventFromCalendarWithDictionnary:(NSDictionary*)options
                        calendar: (EKCalendar *) calendar {
    ...
    // must to remove 60 milliseconds to catch event
    NSTimeInterval _startInterval = [startTime doubleValue] / 1000 - 60; // strip millis
    NSDate *myStartDate = [NSDate dateWithTimeIntervalSince1970:_startInterval];
    
    // must to add 60 milliseconds to catch event
    NSTimeInterval _endInterval = [endTime doubleValue] / 1000 + 60; // strip millis
    NSDate *myEndDate = [NSDate dateWithTimeIntervalSince1970:_endInterval];
...
- (CDVPluginResult*) createEventWithOptionsWithDictionnary:(NSDictionary*) options {
    NSString* title      = [options objectForKey:@"title"];
    NSString* location   = [options objectForKey:@"location"];
    NSString* notes      = [options objectForKey:@"notes"];
    NSNumber* startTime  = [options objectForKey:@"startTime"];
    ...
    if (firstReminderMinutes != nil) {
        EKAlarm *reminder = [EKAlarm alarmWithRelativeOffset:-1*firstReminderMinutes.intValue*60];
        [myEvent addAlarm:reminder];
    }
    
    if (secondReminderMinutes != nil) {
        EKAlarm *reminder = [EKAlarm alarmWithRelativeOffset:-1*secondReminderMinutes.intValue*60];
        [myEvent addAlarm:reminder];
    }
    ...
    if(pluginResult == nil){
        ...
        if (error) {
            ...
            NSLog(@"Added=%@", myEvent.eventIdentifier);
...
- (void)createEventWithOptions:(CDVInvokedUrlCommand*)command {
    ...
    [self.commandDelegate runInBackground: ^{
        CDVPluginResult *pluginResult = [self createEventWithOptionsWithDictionnary:options];
        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
    }];
}

- (void)createEventsWithOptions:(CDVInvokedUrlCommand*)command {
    
    
    [self.commandDelegate runInBackground: ^{
        
        NSArray* jsonArray = command.arguments;
        NSLog(@"nb events to add to calendar = %lu", (unsigned long) jsonArray.count);
        
        NSMutableArray* listIdentifiers = [[NSMutableArray alloc] init];
        CDVPluginResult* pluginResult = nil;
        
        for(int i=0; i < jsonArray.count; i++){
            pluginResult = [self createEventWithOptionsWithDictionnary:[jsonArray objectAtIndex:i]];
            
            NSNumber *CDVCmd_OK = [NSNumber numberWithInt:CDVCommandStatus_OK];
            if( [[pluginResult status] isEqualToNumber:CDVCmd_OK]){
                [listIdentifiers addObject: [pluginResult message]];
            }
            
        }
        
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:[listIdentifiers copy]];
...
- (void) deleteEvents:(CDVInvokedUrlCommand*)command {
    EKCalendar* calendar = self.eventStore.defaultCalendarForNewEvents;
    ...
    if (calendar == nil) {
        ...
        [self.commandDelegate runInBackground: ^{
            ...
            for(int i = 0; i < jsonArray.count; i++){
                [self deleteEventFromCalendarWithDictionnary:[jsonArray objectAtIndex:i] calendar: calendar];
            }
...
- (void) deleteEventsByEventId:(CDVInvokedUrlCommand*)command {
    [self.commandDelegate runInBackground: ^{
        NSArray* jsonArray = command.arguments;
        CDVPluginResult *pluginResult = nil;
        for(int i = 0; i < jsonArray.count; i++){
            EKEvent *event = [self.eventStore eventWithIdentifier:[jsonArray objectAtIndex:i]];
            
            NSError *error = NULL;
            [self.eventStore removeEvent:event span:EKSpanThisEvent error:&error];
            
            if (error) {
                NSLog(@"error with %@", event.eventIdentifier);
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:error.userInfo.description];
            } else {
                NSLog(@"success with %@", event.eventIdentifier);
